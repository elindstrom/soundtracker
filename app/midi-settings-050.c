...
dialog_apply_callback (GtkWidget *notebook)
{
  ...
  case MIDI_SETTINGS_INPUT_PAGE:
    ...
    if (IS_MIDI_DEBUG_ON) {
      g_print( "new input settings: auto %d volume %d channel %d client %d port %d\n",
               new_midi_settings.input.auto_connect,
    ...
    */
    ...
    if ( new_midi_settings.input.auto_connect != midi_settings.input.auto_connect
         || new_midi_settings.input.client != midi_settings.input.client
